...
class RelationsDeepFinder:

	def __init__(self, texts, inputEntities, inputRelations, rel2sent):
		self.texts = texts
	...
	def run(self):
		...
		for paper_number in range(len(self.texts)):

			paper_openie_relations = []
			paper_luanyi_relations = []
			paper_new_relations = []

			for sentence_number in range(len(self.texts[paper_number])):
				new_sentence_relations = []
				sid += 1
				
				sentence_text = self.texts[paper_number][sentence_number]
				...
				#sentenceComplexity = self.getSentenceComplexity(coreNLPparsing)

				sentence_entities = self.inputEntities[paper_number][sentence_number]
				...
				sentence_relations = self.inputRelations
				sentences_openie_relations = [(s,p,o) for (s,p,o) in self.inputRelations[paper_number][sentence_number] if p.startswith('v-')]
				sentences_luanyi_relations = [(s, 'luanyi-' + p, o) for (s,p,o) in self.inputRelations[paper_number][sentence_number] if not p.startswith('v-')]

				id2sent[sid] = sentence_text
				sent2id[sentence_text] = sid
				...
				paper_new_relations += [new_sentence_relations + sentences_openie_relations + sentences_luanyi_relations]

			new_relations += [paper_new_relations]
	
			self.newRelations = new_relations

		self.corenlp.close()
		return self.newRelations
